...
@pytest.mark.parametrize(
    "col_diff_sig", [None, Signature(parts=(ChunkSignature(10, 10, True),))]
)
...
@pytest.mark.parametrize("sig", [None, Signature.aligned(0)])
def test_empty_col_1(monkeypatch, sig):
    monkeypatch.setattr(NumpyDiff, "__eq__", np_raw_diff_eq)
...
def test_dtype_diff_0():
    ...
    assert dtype_diff(
        d1, d2, min_ratio=0, look_field_names=False, look_field_dtypes=True
    ) == Diff(
        ratio=4.0 / 7,
        diffs=[
...
def test_dtype_diff_1():
    ...
    assert dtype_diff(
        d1, d2, look_field_names=True, look_field_dtypes=True, min_ratio=0
    ) == Diff(
        ratio=2.0 / 7,
        diffs=[
            Chunk(data_a=["ix"], data_b=["ix"], eq=True),
            Chunk(
                data_a=["value_1", "value_2", "comment"],
                data_b=["value_3", "value_2"],
                eq=False,
            ),
        ],
    )
...
def test_dtype_diff_2():
    ...
    data_b = np.rec.fromarrays(
        [_b + 10, ["x"] * 9, _b + 30, _b + 90, _b + 80], dtype=d2
    )
    ...
    assert dtype_diff(
        ...
        data_b,
        min_ratio=0,
        look_field_names=False,
        look_field_dtypes=True,
        look_field_data=True,
    ) == Diff(
        ratio=0.4,
        diffs=[
...
def test_dtype_diff_atomic_0():
    ...
    assert dtype_diff(a, b, look_field_data=True) == Diff(
        ratio=1,
        diffs=[
...
def test_dtype_diff_atomic_1():
    ...
    assert dtype_diff(a, b, look_field_data=True, data_min_ratio=0.95) == Diff(
        ratio=0,
        diffs=[
...
def test_diff_record_2(monkeypatch):
    ...
    a = np.rec.fromarrays(
        [
            ...
            np.array(
                [b"alpha", b"beta", b"gamma", b"delta", b"epsilon", b"zeta"], dtype=s32
            ),
    ...
    assert diff(
        ...
        min_ratio=0.6,
        atol=0.2,
        record_compare_names=False,
        record_compare_data=True,
    ) == Diff(
        ratio=2.0 / 3,
        diffs=[
            Chunk(data_a=a[:1], data_b=b[:1], eq=False),
            Chunk(
                data_a=a[1:2], data_b=b[1:2], eq=True, details=[array("i", [1, 1, 1])]
            ),
