...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [
                "--exclude",
                "tests/test_presentation/cases/co2_emissions/",
                "--format",
                "summary",
            ],
            "exclude-2.txt",
...
def test_git_pool(tmp_path):
    ...
    sync_contents(cases / "git/diff-default.txt", text, True)
    assert code is True
...
def test_git_pool_unordered(tmp_path):
    ...
    sync_contents(
        cases / "git/diff-default.txt", text, True, eq=lambda i, j: set(i) == set(j)
    )
    assert code is True
...
def test_readme(test_diff_renders, args, name):
    code, text = process2text(
        [str(cases / "readme/a.txt"), str(cases / "readme/b.txt"), *args]
    )
...
@pytest.mark.parametrize(
    "b",
    ["single-edit", "mosaic-edit", "rm-row", "add-row", "add-col", "rm-col", "not-a"],
)
@pytest.mark.parametrize(
    "args, name",
    [
...
def test_co2_emissions(test_diff_renders, b, args, name):
    code, text = process2text(
        [
            str(cases / "co2_emissions/a.csv"),
            str(cases / f"co2_emissions/b-{b}.csv"),
            "--min-ratio-row",
            "0.6",
            *args,
        ]
...
def test_co2_emissions_4(test_diff_renders, args, name):
    code, text = process2text(
        [
            str(cases / "co2_emissions/b-empty.csv"),
            str(cases / "co2_emissions/b-empty-rm-col.csv"),
            "--mime",
            "text/csv",
            *args,
        ]
...
def test_pd_nans(tmp_path, test_diff_renders):
    code, text = process2text(
        [
            str(cases / "pd_csv_nans/a.csv"),
            str(cases / "pd_csv_nans/b.csv"),
        ]
    )
    sync_contents(cases / "pd_csv_nans/diff.txt", text, test_diff_renders)
    assert code is True
